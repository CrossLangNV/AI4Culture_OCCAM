...
class PageXMLOutput:
        ...
        try:
            ...
            for elem in root.iter():
                if elem.tag.endswith('TextLine'):
                    line_texts = []
                    for unicode_elem in elem.findall('.//Unicode'):
                        if unicode_elem.text is not None:
                            line_texts.append(unicode_elem.text)
                    if line_texts:
                        self.__lines.append(" ".join(line_texts))
                        if self.__log:
    ...
    def update_lines(self, updatedlines: list[str], outfilename: str, wordspecialsep="__SEP__"):
        ...
        try:
            ...
            for elem in root.iter():
                if elem.tag.endswith('TextLine'):
                    textline_elem = elem.find('.//Unicode')
                    if textline_elem is not None and lineid < len(updatedlines):
                        updated_line = updatedlines[lineid]

                        textline_elem.text = updated_line.replace(wordspecialsep, " ")
                        if self.__log:
                            print(f"Updated TextLine {lineid} to: {textline_elem.text}")
...
class NgramDict:
    ...
    def __init__(self, words: list[str], ngramlen=3, max_prop_edit_distance=0.2, max_dictionary_distance_word=2,
        ...
        self.__max_dictionary_edit_distance = min(max_dictionary_distance_word * ngramlen,
                                                  6)  # 7 is default prefix length in SymSpell; this length should exceed max_ditionary_edit_distance
        self.__tmpdir = tmpdir
        self.__log = log
        # enable fuzzy lookup
    ...
    def extract_ngrams(self):
        ...
        for i in range(0, len(self.__words) - self.__ngramlen + 1):
            ...
            if not ngram in self.__ngrams: self.__ngrams[ngram] = list()
        ...
        print(list(map(lambda ngram: ngram.replace(" ", self.__spacesubs) + " 1", self.__ngrams.keys())))
        LineBasedTextFile(tmpfile).write(
            list(map(lambda ngram: ngram.replace(" ", self.__spacesubs) + " 1", self.__ngrams.keys())))
        print("from file", tmpfile)
        print(LineBasedTextFile(tmpfile).read())
    ...
    def get_closest_ngrams(self, ngram: str, samelen=True) -> dict[str]:
        ...
        for suggestion in self.__sym_spell.lookup(ngram.replace(" ", self.__spacesubs), Verbosity.CLOSEST,
                                                  ...
                                                  max_edit_distance=min(self.__max_dictionary_edit_distance, int(len(
                                                      ngram) * self.__max_prop_edit_distance))):
            # print(ngram,suggestion.term)
            matchingngram = suggestion.term.replace(self.__spacesubs, " ")
            # print(matchingngram.split(),self.__ngramlen)
...
class OCRCorrector:
    """
    Finds corrections to OCR output in a PageXML file based on the ngrams in the corresponding manual transcription,
    ...
    def run(self, ocrupdatefile: str, addorig=False):
        """Main method to run OCR correction"""
    ...
    def __get_lineswords(self, filename: str, pagexml=False) -> tuple[list[str], list[str], list[list[str]]]:
        """Read lines and words from the input files"""
        lines = PageXMLOutput(filename).read_lines() if pagexml else LineBasedTextFile(filename).read(stripspaces=True,
    ...
    def __get_ngram_matches(self):
        ...
        """Find ngram matches between OCR and manual transcription"""

        self.__ngrammatches = set()
    ...
    def __get_sets_contiguous_matches(self, minsetsize=1):
        """
        Find sets of ngram matches which cover a contiguous part of the OCR output and a contiguous part of the manual transcription

        """
        restmatches = self.__ngrammatches.copy()
        self.__contigmatchsets = []
            ...
            for ngrammatch in restmatches.copy():
                for i in range(self.__ngramlen):
                    if coveredocrpos[ngrammatch.ocrpos + i] or coveredmanpos[ngrammatch.manpos + i]:
                        ...
                        break
        if self.__log:
            print("Contiguous match sets found:")
            for contigmatchset in self.__contigmatchsets:
                print(contigmatchset)

    def __is_compatible(self, ngrammatch: NgramMatch, contigmatchset: set[NgramMatch]) -> bool:
        """
        set is empty or one of these two conditions applies:
        1) the last words of the OCR ngram occurrence in NGRAMMATCH overlap with the first words of the OCR ngram occurrence of one of the ngrams in CONTIGMATCHSET,
           and the same goes for the manual transcription ngrams
        2) the first words of the OCR ngram occurrence in NGRAMMATCH overlap with the last words of the OCR ngram occurrence of one of the ngrams in CONTIGMATCHSET,
           and the same goes for the manual transcription ngrams
        """
        if not contigmatchset:
            return True
    ...
    def __correct_ocr(self, leftbracket="[", rightbracket="]", correctionchar="~", addorig=False):
        """
        - loop through all sets of ngram matches covering a contiguous part of the OCR output and a contiguous part of the manual transcription and replace words in the OCR output based on the matches in these sets
        - if a word was corrected, prepend it with CORRECTIONCHAR; if ADDORIG is true, prepend it with bracket + original word + CORRECTIONCHAR and add a bracket after word
        - if words are only in manual transcription, append or prepend them to nearest word that was found (exactly or fuzzily) in OCR output, and surround them with double brackets
        - using brackets, flag words in the OCR output that are not covered by any of the match sets
        - however, if a word is not covered by any match set but corresponds to a 3-gram, 4-gram, 5-gram or 6-gram in the manual transcription, correct the OCR word with the latter's words
          and prepend the word with two characters CORRECTIONCHAR; if ADDORIG is true, prepent the original word to the first word in the ngram and surround both word with brackets
        """
        self.__correctedwords = self.__ocrwords.copy()
        coveredocrpos = [False] * len(self.__ocrwords)
        man2ocrpos = [-1] * len(self.__mantranswords)

        # Correct words based on contiguous match sets
        for contigmatchset in self.__contigmatchsets:
            for ngrammatch in contigmatchset:
                for i in range(self.__ngramlen):
                    if not coveredocrpos[ngrammatch.ocrpos + i] and self.__correctedwords[ngrammatch.ocrpos + i] != \
                        ...
                        corrected_word = (leftbracket + self.__correctedwords[
                            ngrammatch.ocrpos + i] if addorig else '') + correctionchar + self.__mantranswords[
                                             ngrammatch.manpos + i] + (rightbracket if addorig else '')
                        self.__correctedwords[ngrammatch.ocrpos + i] = corrected_word
                        if self.__log:
                            print(f"Corrected '{self.__ocrwords[ngrammatch.ocrpos + i]}' to '{corrected_word}'")
                    coveredocrpos[ngrammatch.ocrpos + i] = True
                    man2ocrpos[ngrammatch.manpos + i] = ngrammatch.ocrpos + i

        # Handle words not covered by any match set
        manngramdicts = [NgramDict(self.__mantranswords, ngramlen=i, max_prop_edit_distance=0.5) for i in range(3, 6)]
        for i in range(len(self.__ocrwords)):
            if not coveredocrpos[i]:
                ...
                if manocrbegpos == -1:
                    ...
                    corrected_phrase = (leftbracket + self.__correctedwords[
                        i] if addorig else '') + correctionchar + correctionchar + self.__mantranswords[
                                           manocrbegpos] + (
                                           rightbracket if addorig else '') + self.__wordspecialsep + correctionchar + correctionchar + (

                                               self.__wordspecialsep + correctionchar + correctionchar).join(
                        self.__mantranswords[manocrbegpos + 1:manocrendpos + 1])
                    self.__correctedwords[i] = corrected_phrase
                    for j in range(manocrbegpos, manocrendpos + 1):
                        man2ocrpos[j] = i

        # Add missing words from manual transcription
        i = 0
        ...
        if self.__log:
            print("Corrected words:", self.__correctedwords)

    def __correct_ocr_diffwordnum(self, ocrword, manngramdicts, man2ocrpos) -> tuple[int, int]:
        """Correct OCR words with different word numbers"""
        for manngramdict in manngramdicts:
            matchdict = manngramdict.get_closest_ngrams(ocrword, samelen=False)
            for manngram in matchdict:
                for manpos in self.__manngramdict.get_ngram_pos(manngram):

                    if all(man2ocrpos[j] == -1 for j in range(manpos, manpos + manngramdict.ngramlen)):
                        if self.__log:
                            print("Diffwordnum", ocrword, manngram)
                        return manpos, manpos + manngramdict.ngramlen - 1
        return -1, -1

    def __correct_pagexml(self, outfilename: str):
        """Update the PageXML file with corrected lines"""
        wordcount = 0
        correctedlines = []
        for linewords in self.__ocrlineswords:
            correctedlines.append(" ".join(self.__correctedwords[wordcount:wordcount + len(linewords)]))
            wordcount += len(linewords)
        if self.__log:
            print("Corrected lines:", correctedlines)
    ...
    @property
    def ngrammatches(self) -> set[NgramMatch]:
        return self.__ngrammatches

    @property
    def contigmatchsets(self) -> set[set]:
    ...
    def __repr__(self):
        return f"OCRCorrector(mantransfilename=%r,ocrfilename1=%r,ngramlen=%r,log=%r)" % (
    ...
    def __str__(self):
        ...
        maxcount = 30
        if self.__ocrwords != None: result += "\nOCR words: " + str(self.__ocrwords[0:maxcount])
        if self.__mantranswords != None: result += "\nmanual transcription words: " + str(
            self.__mantranswords[0:maxcount])
        if self.__correctedwords != None: result += "\ncorrected OCR words: " + str(self.__correctedwords[0:maxcount])
...
if __name__ == "__main__":
    ...
    # if a fourth argument "1" is specified, add original words to corrections in PageXML output
    OCRCorrector(ocroutfile, mantrans).run(outfile, len(sys.argv) == 5)
